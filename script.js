// ============================================
// 🐻 Cuddly AI v1.0 - Data File Storage
// ============================================
...
(function resetAllData() {
    try {
        ...
        console.log('🗑️ All data cleared to ZERO state');
    } catch (e) {
...
var DATA_FILE_NAME = 'cuddly-data.json';
...
function saveDataToFile() {
    try {
        ...
        console.error('Could not save data to file:', e);
        return false;
    }
}
...
function loadDataFromFile() {
    return new Promise(function(resolve, reject) {
        try {
            ...
            input.onchange = function(e) {
                ...
                reader.onload = function(ev) {
                    try {
                        ...
                        if (data.apiKeys) API_KEYS = data.apiKeys;
            ...
            reject('Could not load data: ' + e.message);
        }
    });
}
...
function saveAllToStorage() {
    try {
        ...
        localStorage.setItem('cuddly-current-user', JSON.stringify(currentUser));
        ...
        for (var key in API_KEYS) {
            if (API_KEYS[key]) {
                localStorage.setItem('cuddly-' + key + '-key', API_KEYS[key]);
            }
        }
...
function loadAllFromStorage() {
    try {
        var savedUsers = localStorage.getItem('cuddly-users');
        var savedCurrentUser = localStorage.getItem('cuddly-current-user');
        var savedChats = localStorage.getItem('cuddly-chats');
        var savedHistory = localStorage.getItem('cuddly-history');
        var savedTheme = localStorage.getItem('cuddly-theme');
        ...
        // API keys
        var apiProviders = ['deepseek', 'gemini', 'claude', 'openai'];
        apiProviders.forEach(function(provider) {
            var key = localStorage.getItem('cuddly-' + provider + '-key');
...
// ============================================
// EXPORT DATA (Faylga saqlash tugmasi - ADMIN UCHUN)
// ============================================

// Bu tugma faqat admin uchun, foydalanuvchi ko'rmaydi
// Agar kerak bo'lmasa, bu qismni olib tashlang

function addAdminExportButton() {
    // Faqat admin uchun - foydalanuvchiga ko'rsatilmaydi
    // Bu tugmani faqat siz bilasiz
    console.log('🔑 Admin: Use saveDataToFile() to export data');
    console.log('🔑 Admin: Use loadDataFromFile() to import data');
    
    // Konsoldan ishlatish uchun
    window.saveData = saveDataToFile;
    window.loadData = loadDataFromFile;
    
    console.log('📌 Commands:');
    console.log('  saveData()  - Save all data to file');
    console.log('  loadData()  - Load data from file');
}
...
// ============================================
// ADMIN FUNCTIONS (KONSOLDAN ISHLATISH UCHUN)
// ============================================
...
window.loadData = function() {
    loadDataFromFile().then(function() {
...
function initApp() {
    ...
    console.log('  saveData()  - Save all data to JSON file');
    console.log('  loadData()  - Load data from JSON file');
    console.log('  viewData()  - View all data in console');
}
...
console.log('📌 To save data: saveData()');
console.log('📌 To load data: loadData()');